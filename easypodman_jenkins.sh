...
export INPUT_FILE=${local_input_files}${params.Inputfilename}
export JOB_ID=${params.Jobid}
...
containerID=$(podman run -d --replace --name  ${containerName}  ${imageName} sleep infinity)
#containerID=$(podman --storage-opt ignore_chown_errors=true run --env-file ${ENV_FILE} -dt ${imageName} /bin/bash)
echo containerID = ${containerID}
#sleep infinity
#[ -n "$SCRIPT" "$JOB_ID" "$INPUT_FILE" ] && sh "$SCRIPT" '
#podman exec ${containerName} find / -name pilot.sh 2>/dev/null
#podman exec ${containerName} /autodx/sw_tools/SW_MW/entry/pilot.sh $JOB_ID $INPUT_FILE
...
chmod -R a+rwx ${WORKSPACE}/Test_results
echo "Copying {container}:/sr/output/* to output..."
podman cp "${containerID}:/sr/output" ${WORKSPACE}/Test_results/
echo ""
